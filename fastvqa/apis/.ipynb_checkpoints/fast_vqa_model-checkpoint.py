...
from fastvqa.datasets import SampleFrames, get_spatial_fragments
...
class VQAModel:
    ...
    def __call__(self, x, verbose=False, local_scores=False, return_fragments=False):
        ...
        x = x[:, sampled_frames]
        x = get_spatial_fragments(
            x, self.fragments, self.fragments, aligned=self.aligned
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-v",
        type=str,
        default="test_video/A001.mp4",
        help="path of the demo video",
    )
    parser.add_argument(
        "-d",
        type=str,
    ...
    try:
        ...
        input_video = (
            (decord.VideoReader(args.v)[:] / 255.0).permute(3, 0, 1, 2).to(args.d)
        )
    except:
        ## compatible version for Apple M1 which does not support decord
        from torchvision.io import read_video

        input_video = (read_video(args.v)[0] / 255.0).permute(3, 0, 1, 2).to(args.d)
