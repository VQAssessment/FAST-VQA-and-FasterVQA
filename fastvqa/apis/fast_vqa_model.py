...
class VQAModel:
    def __init__(self, pretrained=False, pretrained_path=None, model_type='fast', device='cpu'):
        
        self.sampler = SampleFrames(32, 2, 4) if model_type=='fast' else SampleFrames(16, 2, 4)
        self.fragments = 7 if model_type=='fast' else 4
        backbone_hp = dict(window_size=(4,4,4),
                         frag_biases=[False,False,True,False]) if model_type=='faster' else dict()
        ...
        print(f'''Successfully loaded pretrained=[{pretrained}] {model_type}-vqa model from pretrained_path=[{pretrained_path}].
...
def deep_end_to_end_vqa(pretrained=False, pretrained_path=None, model_type='fast', device='cpu'):
    return VQAModel(pretrained, pretrained_path=pretrained_path, model_type=model_type, device=device)
