...
def generate_dataset(args, dataset, seed=42, dataset_hp=dict()):
    ...
    train_infos = f"examplar_data_labels/train_labels.txt"
            ...
            train_infos = f"examplar_data_labels/{training_dataset_name}/labels.txt"
    ...
    training_set = VQAInferenceDataset(
...
def training_epoch(ft_loader, model, optimizer, device):
    model.train()
...
def main():
    ...
    parser.add_argument(
        "-lep", "--l_num_epochs", type=int, default=10, help="linear training epochs"
    )
    parser.add_argument(
        "-ep", "--num_epochs", type=int, default=20, help="training epochs"
    )
    ...
    # torch.save({'results': bests_}, f'{args.save_dir}/results_{args.model_type}_training_{args.dataset.lower()}_s{args.fsize}*{args.fsize}_ens{args.famount}{"" if not args.from_ar else "_from_ar"}.pkl')
    ...
    for i in range(10):
        ...
        if args.from_ar:
            ...
            if args.fsize != 32:
                raise NotImplementedError(
                    "Version 0.3.0 only supports 32*32 training on fragments."
                )
        ...
        print(
            f"Before the training process on {args.dataset} with {len(val_loader)} videos, \nthe accuracy of the model is as follows:\n  SROCC: {best_[0]:.4f}\n  PLCC:  {best_[1]:.4f}\n  KROCC: {best_[2]:.4f}\n  RMSE:  {best_[3]:.4f}."
        )
        ...
        for epoch in range(args.l_num_epochs):
            ...
            training_epoch(ft_loader, model, optimizer, device)
            best_ = inference_set(val_loader, model, device, best_)
        ...
        for epoch in range(args.num_epochs):
            print(f"Split {i}, training Epoch {epoch}:")
            training_epoch(ft_loader, model, optimizer, device)
            best_ = inference_set(val_loader, model, device, best_)

        print(
            f"For the training process on {args.dataset} with {len(val_loader)} videos, \nthe best validation accuracy of the model is as follows:\n  SROCC: {best_[0]:.4f}\n  PLCC:  {best_[1]:.4f}\n  KROCC: {best_[2]:.4f}\n  RMSE:  {best_[3]:.4f}."
        )
        ...
        torch.save(
            {"results": bests_},
            f'{args.save_dir}/results_{args.model_type}_training_{args.dataset.lower()}_s{args.fsize}*{args.fsize}_ens{args.famount}{"" if not args.from_ar else "_from_ar"}.pkl',
        )
